...
/*=============================================================================
	DestructibleComponent.cpp: URemoteDestructibleComponent methods.
=============================================================================*/
...
URemoteDestructibleComponent::URemoteDestructibleComponent(const FObjectInitializer& ObjectInitializer)
...
void URemoteDestructibleComponent::Serialize(FArchive& Ar)
{
	Super::Serialize(Ar);
...
#if WITH_EDITOR
void URemoteDestructibleComponent::PostEditChangeProperty(struct FPropertyChangedEvent& PropertyChangedEvent)
{
...
FBoxSphereBounds URemoteDestructibleComponent::CalcBounds(const FTransform& LocalToWorld) const
{
#if WITH_APEX
...
void URemoteDestructibleComponent::OnUpdateTransform(EUpdateTransformFlags UpdateTransformFlags, ETeleportType Teleport)
{
	...
	{
		logOrEnsureNanError(TEXT("URemoteDestructibleComponent:OnUpdateTransform found NaN in CurrentLocalToWorld: %s"), *CurrentLocalToWorld.ToString());
		return;
	}
#endif
	...
	{
		UE_LOG(LogPhysics, Log, TEXT("URemoteDestructibleComponent::SendPhysicsTransform : Non-uniform scale factor (%s) can cause physics to mismatch for %s  SkelMesh: %s"), *MeshScale3D.ToString(), *GetFullName(), SkeletalMesh ? *SkeletalMesh->GetFullName() : TEXT("NULL"));
	}
#endif
...
void URemoteDestructibleComponent::OnCreatePhysicsState()
{
	...
	{
		UE_LOG(LogPhysics, Log, TEXT("URemoteDestructibleComponent::OnCreatePhysicsState(): APEX must be enabled to init URemoteDestructibleComponent physics."));
		return;
	}

	if (ApexDestructibleActor != NULL)
	{
		UE_LOG(LogPhysics, Log, TEXT("URemoteDestructibleComponent::OnCreatePhysicsState(): NxDestructibleActor already created."));
		return;
	}
	...
	{
		UE_LOG(LogPhysics, Log, TEXT("URemoteDestructibleComponent::OnCreatePhysicsState(): No DestructibleMesh or missing ApexDestructibleAsset."));
		return;
	}
	...
	//NvParameterized::Interface* ActorParams = TheDestructibleMesh->GetDestructibleActorDesc(PhysMat); // <----------- GetDestructibleActorDesc gives unresolved external
#endif
...
void URemoteDestructibleComponent::OnDestroyPhysicsState()
{
#if WITH_APEX
	if (ApexDestructibleActor != NULL)
	{
...
UBodySetup* URemoteDestructibleComponent::GetBodySetup()
{
	if (SkeletalMesh != NULL)
	{
		UDestructibleMesh* TheDestructibleMesh = GetDestructibleMesh();
...
bool URemoteDestructibleComponent::CanEditSimulatePhysics()
{
	// if destructiblemeshcomponent, we will allow it always
	return true;
}

void URemoteDestructibleComponent::AddImpulse(FVector Impulse, FName BoneName /*= NAME_None*/, bool bVelChange /*= false*/)
{
#if WITH_APEX
...
void URemoteDestructibleComponent::AddImpulseAtLocation(FVector Impulse, FVector Position, FName BoneName /*= NAME_None*/)
{
#if WITH_APEX
	ExecuteOnPhysicsReadWrite([&]
	{
...
void URemoteDestructibleComponent::AddForce(FVector Force, FName BoneName /*= NAME_None*/, bool bAccelChange /* = false */)
{
#if WITH_APEX
	ExecuteOnPhysicsReadWrite([&]
	{
...
void URemoteDestructibleComponent::AddForceAtLocation(FVector Force, FVector Location, FName BoneName /*= NAME_None*/)
{
#if WITH_APEX
	ExecuteOnPhysicsReadWrite([&]
	{
...
void URemoteDestructibleComponent::AddRadialImpulse(FVector Origin, float Radius, float Strength, ERadialImpulseFalloff Falloff, bool bVelChange)
{
#if WITH_APEX
...
void URemoteDestructibleComponent::AddRadialForce(FVector Origin, float Radius, float Strength, ERadialImpulseFalloff Falloff, bool bAccelChange /* = false */)
{
#if WITH_APEX
...
void URemoteDestructibleComponent::ReceiveComponentDamage(float DamageAmount, FDamageEvent const& DamageEvent, AController* EventInstigator, AActor* DamageCauser)
{
...
#if WITH_APEX
void URemoteDestructibleComponent::SpawnFractureEffectsFromDamageEvent(const apex::DamageEventReportData& InDamageEvent)
{
...
void URemoteDestructibleComponent::OnDamageEvent(const apex::DamageEventReportData& InDamageEvent)
{
...
void URemoteDestructibleComponent::OnVisibilityEvent(const apex::ChunkStateEventData & InVisibilityEvent)
{
...
bool URemoteDestructibleComponent::IsFracturedOrInitiallyStatic() const
{
...
bool URemoteDestructibleComponent::ExecuteOnPhysicsReadOnly(TFunctionRef<void()> Func) const
{
#if WITH_APEX
	if (ApexDestructibleActor)
	{
...
bool URemoteDestructibleComponent::ExecuteOnPhysicsReadWrite(TFunctionRef<void()> Func) const
{
#if WITH_APEX
	if (ApexDestructibleActor)
	{
...
void URemoteDestructibleComponent::RefreshBoneTransforms(FActorComponentTickFunction* TickFunction)
{
}

void URemoteDestructibleComponent::SetDestructibleMesh(class UDestructibleMesh* NewMesh)
{
#if WITH_APEX
...
class UDestructibleMesh * URemoteDestructibleComponent::GetDestructibleMesh()
{
	return Cast<UDestructibleMesh>(SkeletalMesh);
}

void URemoteDestructibleComponent::SetSkeletalMesh(USkeletalMesh* InSkelMesh, bool bReinitPose)
{
	if (InSkelMesh != NULL && !InSkelMesh->IsA(UDestructibleMesh::StaticClass()))
	{
		...
		UE_LOG(LogPhysics, Log, TEXT("URemoteDestructibleComponent::SetSkeletalMesh(): Passed-in USkeletalMesh (%s) must be a UDestructibleMesh.  SkeletalMesh not set."), *InSkelMesh->GetPathName());
		return;
	}
	...
	if (TheDestructibleMesh != NULL && TheDestructibleMesh->ApexDestructibleAsset == NULL)
	{
		UE_LOG(LogPhysics, Log, TEXT("URemoteDestructibleComponent::SetSkeletalMesh(): Missing ApexDestructibleAsset on '%s'."), *InSkelMesh->GetPathName());
		return;
	}
...
FTransform URemoteDestructibleComponent::GetSocketTransform(FName InSocketName, ERelativeTransformSpace TransformSpace) const
{
...
#if WITH_APEX
void URemoteDestructibleComponent::Pair(int32 ChunkIndex, PxShape* PShape)
{
...
void URemoteDestructibleComponent::SetChunkVisible(int32 ChunkIndex, bool bInVisible)
{
#if WITH_APEX
...
#if WITH_APEX
void URemoteDestructibleComponent::UpdateDestructibleChunkTM(const TArray<const PxRigidActor*>& ActiveActors)
{
	...
	TMap<URemoteDestructibleComponent*, TArray<FUpdateChunksInfo> > ComponentUpdateMapping;
			...
			if (GApexModuleDestructible->owns(RigidActor) && DestructibleChunkInfo->OwningComponent.IsValid())
			{
				...
				for (int32 ShapeIdx = 0; ShapeIdx < Shapes.Num(); ++ShapeIdx)
				{
					...
					if (apex::DestructibleActor* DestructibleActor = GApexModuleDestructible->getDestructibleAndChunk(Shape, &ChunkIndex))
					{
						...
						const physx::PxTransform Transform(ChunkPoseRT);
						if (URemoteDestructibleComponent* DestructibleComponent = Cast<URemoteDestructibleComponent>(FPhysxUserData::Get<UPrimitiveComponent>(DestructibleActor->userData)))
						{
	...
	{
		URemoteDestructibleComponent* DestructibleComponent = It.Key();
...
void URemoteDestructibleComponent::SetChunksWorldTM(const TArray<FUpdateChunksInfo>& UpdateInfos)
{
...
void URemoteDestructibleComponent::SetChunkWorldRT(int32 ChunkIndex, const FQuat& WorldRotation, const FVector& WorldTranslation)
{
	// Bone 0 is a dummy root bone
	const int32 BoneIndex = ChunkIdxToBoneIdx(ChunkIndex);
...
void URemoteDestructibleComponent::ApplyDamage(float DamageAmount, const FVector& HitLocation, const FVector& ImpulseDir, float ImpulseStrength)
{
#if WITH_APEX
	if (ApexDestructibleActor != NULL)
	{
...
void URemoteDestructibleComponent::ApplyRadiusDamage(float BaseDamage, const FVector& HurtOrigin, float DamageRadius, float ImpulseStrength, bool bFullDamage)
{
#if WITH_APEX
	if (ApexDestructibleActor != NULL)
	{
...
int32 ExportPxActorGeometry(const PxRigidDynamic* PActor, TArray<PxShape*>& Shapes, FNavigableGeometryExport& GeomExport)
{
	...
	for (PxU32 ShapeIndex = 0; ShapeIndex < RetrievedShapesCount; ++ShapeIndex, ++ShapePtr)
	{
		if (*ShapePtr != NULL)
		{
			...
			default:
			{
				UE_LOG(LogPhysics, Log, TEXT("URemoteDestructibleComponent::DoCustomNavigableGeometryExport(): unhandled PxGeometryType, %d.")
...
bool URemoteDestructibleComponent::DoCustomNavigableGeometryExport(FNavigableGeometryExport& GeomExport) const
{
...
void URemoteDestructibleComponent::Activate(bool bReset/*=false*/)
{
...
void URemoteDestructibleComponent::Deactivate()
{
...
void URemoteDestructibleComponent::SetCollisionResponseToChannel(ECollisionChannel Channel, ECollisionResponse NewResponse)
{
#if WITH_APEX
	if (ApexDestructibleActor)
	{
...
void URemoteDestructibleComponent::SetCollisionResponseToAllChannels(ECollisionResponse NewResponse)
{
#if WITH_APEX
	if (ApexDestructibleActor)
	{
...
void URemoteDestructibleComponent::SetCollisionResponseToChannels(const FCollisionResponseContainer& NewReponses)
{
#if WITH_APEX
	if (ApexDestructibleActor)
	{
...
bool URemoteDestructibleComponent::ShouldUpdateTransform(bool bLODHasChanged) const
{
	// We do not want to update bone transforms before physics has finished
	return false;
}

bool URemoteDestructibleComponent::LineTraceComponent(FHitResult& OutHit, const FVector Start, const FVector End, const FCollisionQueryParams& Params)
{
	bool bHaveHit = false;
#if WITH_APEX
	if (ApexDestructibleActor != NULL)
...
bool URemoteDestructibleComponent::SweepComponent(FHitResult& OutHit, const FVector Start, const FVector End, const FQuat& ShapeWorldRotation, const FCollisionShape &CollisionShape, bool bTraceComplex/*=false*/)
{
	bool bHaveHit = false;
#if WITH_APEX
	if (ApexDestructibleActor != NULL)
...
#if WITH_APEX
void URemoteDestructibleComponent::SetupFakeBodyInstance(physx::PxRigidActor* NewRigidActor, int32 InstanceIdx, FFakeBodyInstanceState* PrevState)
{
...
void URemoteDestructibleComponent::ResetFakeBodyInstance(FFakeBodyInstanceState& PrevState)
{
...
void URemoteDestructibleComponent::WakeRigidBody(FName BoneName /* = NAME_None */)
{
#if WITH_APEX
	ExecuteOnPhysicsReadWrite([&]
	{
...
void URemoteDestructibleComponent::SetSimulatePhysics(bool bSimulate)
{
#if WITH_APEX
	ExecuteOnPhysicsReadWrite([&]
	{
...
void URemoteDestructibleComponent::SetEnableGravity(bool bGravityEnabled)
{
...
FBodyInstance* URemoteDestructibleComponent::GetBodyInstance(FName BoneName /*= NAME_None*/, bool) const
{
#if WITH_APEX
	if (ApexDestructibleActor != NULL)
	{
		...
		const_cast<URemoteDestructibleComponent*>(this)->SetupFakeBodyInstance(PActor, BoneIdx);
	}
#endif // WITH_APEX
...
bool URemoteDestructibleComponent::IsAnySimulatingPhysics() const
{
...
bool URemoteDestructibleComponent::IsChunkLarge(PxRigidActor* ChunkActor) const
{
#if WITH_APEX
	check(ChunkActor);
...
void URemoteDestructibleComponent::OnActorEnableCollisionChanged()
{
	ECollisionEnabled::Type NewCollisionType = GetBodyInstance()->GetCollisionEnabled();
	SetCollisionEnabled(NewCollisionType);
}

void URemoteDestructibleComponent::SetCollisionEnabled(ECollisionEnabled::Type NewType)
{
#if WITH_APEX
	ExecuteOnPhysicsReadWrite([&]
	{
...
void URemoteDestructibleComponent::SetCollisionResponseForActor(PxRigidDynamic* Actor, int32 ChunkIdx, const FCollisionResponseContainer* ResponseOverride /*= NULL*/)
{
#if WITH_APEX
	if (ApexDestructibleActor == NULL)
	{
...
void URemoteDestructibleComponent::SetCollisionResponseForShape(PxShape* Shape, int32 ChunkIdx)
{
#if WITH_APEX
...
void URemoteDestructibleComponent::SetMaterial(int32 ElementIndex, UMaterialInterface* Material)
{
